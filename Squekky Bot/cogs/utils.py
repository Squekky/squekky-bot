...
class Utils(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        fun = ["`hangman`", "`disasters`", "`countries`", "`capitals`", "`states`", "`teams`", "`companies`",
               "`yahtzee`", "`roll`", "`wordle`", "`nerdle`", "`stats`", "`leaderboard`", "`leaderboards`"]
        ...
        embed.add_field(name="Fun", value=f"{', '.join(fun)}", inline=False)
    ...
    async def help_rolecheck(self, ctx):
        """ SERVERINFO HELP """
        embed = await get_help("rolecheck (role ID)")
