...
class Flags(commands.Cog):
    ...
    async def flags(self, ctx, mode=None):
        ...
        def game_over(color):
            accuracy = round(((score / guessed) * 100), 2) if guessed > 0 else 0
            embed = discord.Embed(
                title=f"Final Score: {score}/{guessed} [{accuracy}%]",
                colour=color
            )
        ...
        flag_file = discord.File(f"./files/flags/{full_flag_image}", filename=flag_image)
        embed = flag_embed(0xD780FF)
        embed.set_thumbnail(url=f"attachment://{flag_image}")
        ...
        for flag in flag_names:
            ...
            try:  # Continue to prompt the user for guesses until they quit or finish the game
                ...
                guessed -= 1
                await ctx.send(embed=game_over(0xFF6675))
                self.flags.reset_cooldown(ctx)
                break
            if response.upper() == 'EXIT':  # Break the loop and end the game if the user types "exit"
                guessed -= 1
                await ctx.send(embed=game_over(0x66FFCC))
                break
            elif rmv_punc(response.lower().replace(" ", "")) == rmv_punc(flag.lower().replace(" ", "")):
                score += 1
    ...
    @flags.error
    async def flags_error(self, ctx, error):
